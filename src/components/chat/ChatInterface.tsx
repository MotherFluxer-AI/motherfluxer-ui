...
export const ChatInterface: React.FC = () => {
  ...
  const wsUrls = [
    'wss://beeef73badbe-8000.proxy.runpod.net/ws',
    'ws://beeef73badbe-8000.proxy.runpod.net/ws',
    'wss://beeef73badbe-8000.proxy.runpod.net',
    'ws://beeef73badbe-8000.proxy.runpod.net'
  ];
  ...
  useEffect(() => {
    ...
    const connectWebSocket = () => {
      ...
      console.log(`Attempting to connect to WebSocket at ${currentUrl}...`);
      ...
      try {
        ...
        ws.onerror = (err) => {
          ...
          setError(`Failed to connect to model service at ${currentUrl}`);
        };
        ...
        ws.onclose = (event) => {
          ...
          if (event.code !== 1000 && !isAuthenticated) {
            if (urlIndex < wsUrls.length - 1) {
              ...
              // If we've tried all URLs, start over with connection attempts
              setUrlIndex(0);
    ...
    <div className="w-full max-w-2xl mx-auto p-4">
      {!isAuthenticated && !error && (
        <div className="mb-4 p-2 bg-yellow-100 text-yellow-800 rounded">
          ...
          <div className="text-xs mt-1">
            Trying URL: {wsUrls[urlIndex]}
            {connectionAttempts > 0 ? ` (Attempt ${connectionAttempts + 1}/4)` : ''}
          </div>
        </div>
      )}
      {isAuthenticated && (
      ...
      {error && (
        <div className="mb-4">
          ...
          <button 
            onClick={() => {
              // Try the next URL first
              if (urlIndex < wsUrls.length - 1) {
                setUrlIndex(urlIndex + 1);
              } else {
                // If we've tried all URLs, increment connection attempts
                setUrlIndex(0);
                setConnectionAttempts(prev => prev + 1);
              }
            }} 
